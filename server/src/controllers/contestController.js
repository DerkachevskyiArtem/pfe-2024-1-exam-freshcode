...
module.exports.getCustomersContests = async (req, res, next) => {
  const {
    query: { userId, status = 'active', limit = 10, offset = 0 },
  } = req;
...
module.exports.getContests = (req, res, next) => {
  ...
  const BoolOwnEntries = ownEntries === 'true';
  ...
  db.Contests.findAll({
    ...
    limit: limit,
    offset: offset ? req.body.offset : 0,
    include: [
      {
        model: db.Offers,
        required: BoolOwnEntries,
        where: BoolOwnEntries ? { userId: req.tokenData.userId } : {},
        attributes: ['id'],
      },
    ],
  })
